...
### 11. LINE への動画添付はクリップボード貼り付けでは効かない。📎 ボタンを使う

2026-07-31 05:10 に確認。`osascript -e 'set the clipboard to (POSIX file "…")'` でファイル参照を
クリップボードへ入れ（`clipboard info` は `«class furl»` を返し、書き込み自体は成功している）、
入力欄にフォーカスした状態で Cmd+V を送っても、**LINE の入力欄には何も現れない**。
テキストの貼り付けは同じ方法で効くので、ファイル（furl）だけが受け付けられない。

正しい手順:

1. 添付したいファイルを `~/Downloads` へコピーする（ファイル選択パネルの既定表示位置）
2. 入力欄左下の 📎（クリップ）ボタンをクリックしてファイル選択パネルを開く
3. **シングルクリックで選択**し、目的のファイル名がハイライトされていることをキャプチャで確認する
   （ダブルクリックは即送信になるので、宛先ヘッダーの確認前に押してはいけない）
4. 「開く」を押す。押した時点で送信が始まる（別途 Enter は不要）
5. アップロード率 → サムネイル → 尺表示（例 `00:11`）の順に画面で確認する

似た名前のファイルが並ぶ（`hero-veo-preview.mp4` / `hero-seedance-preview.mp4`）ので、
選択後のハイライト文字列を必ず読んでから「開く」を押すこと。

補足: `cliclick` の座標は Retina でもロジカルポイントで指定してよい。
スクリーンショット（`screencapture` の出力は物理ピクセル）から座標を拾う場合は、
`物理幅 / 2` がロジカル幅になる換算を挟むこと。`screencapture` / `sips` は
サンドボックス下では PATH に無いので `/usr/sbin/screencapture` / `/usr/bin/sips` と絶対パスで呼ぶ。
